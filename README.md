# dsh-gitp — Git 面板（DeepSeek Harness WebUI 插件）
...
```sh
dsh plugin --profile web add dsh-gitp
```
...
> `gitp` 这一行）。之后**重启** webui 进程生效（Node 模块缓存不会热替换旧代码）。
...
- 面板调用宿主 `/gitp/*` 路由，宿主侧按会话沙箱策略执行；不会放宽你当前的文件权限模式
...
| 半 | 文件 | 机制 |
|---|---|---|
| Host 路由 | `index.js` | 包主入口；`inject: [webServer, shell]` 等 `webServer` 就绪后注册 `/gitp/status|diff|commit|push|automessage` 五个 exact 路由 |
| 浏览器面板 | `client.js` | `exports["./client"]` + `dsh.client.platform: web`，由 `dsh-client-modules` 扫描发现，经 `/plugins/gitp/client.js` 注入页面，注册到 `shell.overlay` 插槽（id `git-panel`, order 90） |

`cordis.patch.yml` 是 bundle 层：profile 列出本 bundle 时插入一行 `gitp`。
...
```sh
dsh plugin --profile web add ./dsh-gitp     # 重装（或先 remove）
dsh --profile web --dump-config | grep gitp # 确认行存在
# 重启 webui 进程
```
